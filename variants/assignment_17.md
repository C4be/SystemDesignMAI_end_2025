...
17. **Обнаружение языка ненависти в комментариях:** Спроектируйте систему для обнаружения и пометки языка ненависти в комментариях на новостном сайте. Сосредоточьтесь на обработке текста и учете контекста.
...
- Задержка (latency) ответа не должна превышать **385 мс**.
- Система должна обслуживать **3,207,228 активных пользователей в день (DAU)**, с пиковой нагрузкой в **18,225 запросов в секунду (RPS)**.
